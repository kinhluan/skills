...
When multi-agent collaboration is explicitly requested and role labels add clarity, use:
...
- **Escalate:** If no consensus is reached, present the decision owner with evidence, tradeoffs, and the reversible next step. An agent role does not override the user or repository owner.
