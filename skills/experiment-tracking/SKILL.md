...
Format: **bold** = best only when the comparison is statistically and
practically meaningful. Choose the number of independent runs from a precision
or power analysis informed by pilot variance and compute constraints; report
the run count, seeds, uncertainty interval, and any exploratory limitation.
