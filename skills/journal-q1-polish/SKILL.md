...
### 3.3 Independent-Run and Sample-Size Justification

Include in experimental setup:

```
We report the distribution over N independent runs. N was selected
from [power/precision target] using [pilot or prior variance], with
[effect/interval target] and [compute constraints]. Seeds and run-level
results are available in [artifact].
```

Do not use `n ≥ 30`, a fixed seed table, or the Central Limit Theorem as a universal justification. Independence, estimator behavior, variance, effect size, model assumptions, and the unit of analysis still matter. Use a pilot or prior evidence to plan runs; disclose when compute constraints make the analysis exploratory.

### 3.4 Statistical Tests

| Claim | Required evidence |
|-------|-------------------|
| "outperforms" / "better than" | Defined estimand, effect size, uncertainty, and an assumption-compatible comparison |
| "equivalent" / "non-inferior" | Predefined margin and equivalence/non-inferiority analysis |
| "robust to hyperparameter" | Prespecified sensitivity analysis across a justified range |

Confidence-interval overlap alone is not an equivalence test. Adjust for multiple comparisons or label exploratory analyses.

### 3.5 Results Table Checklist

- [ ] **All cells** in ablation study show `mean ± std` (not just main results table)
- [ ] Independent-run/sample count and its power, precision, variance, or resource justification are stated
- [ ] Effect sizes and confidence intervals are shown; significance markers define the test and multiplicity correction
- [ ] Best result **bolded**, second-best *underlined*
- [ ] Baselines are rerun under the same evaluation protocol when feasible
- [ ] Values quoted from original papers are isolated, cited, and labeled non-comparable when data, preprocessing, hardware, budget, or metrics differ
...
- [ ] Verified current scope, indexing claims, policies, fees, and any published statistics on official sources
