...
- Define the estimand and unit of analysis before choosing a test.
- Estimate sample size or independent-run count from a power/precision analysis, expected effect, pilot variance, and compute budget.
- Match paired tests to genuinely paired observations; check assumptions instead of choosing a t-test or Wilcoxon test by habit.
- Report effect sizes and confidence intervals alongside multiplicity-adjusted inferential tests when applicable.
- For stochastic systems, publish the run-level results and seed-generation procedure; do not claim that a universal seed count guarantees reliability.
...
- [ ] Baselines run under the same evaluation protocol and a justified resource policy where feasible
- [ ] Any result copied from a paper is isolated and labeled as non-comparable unless protocols match
...
- Sample/run count: [power, precision, pilot variance, or other justification]
- Assumption checks and test/model: [...]
...
- Use [references/experiment-templates.md](references/experiment-templates.md) for reusable protocol and result-table templates.
