...
# Detailed Business and Product Leadership Guide

> Reference status: This file preserves extended examples. Treat every numeric threshold, tool, vendor, and market claim as illustrative; calibrate it to current product evidence before use. The top-level `SKILL.md` is authoritative when guidance conflicts.
...
|---|---|---|
| **Situation** (frequency) | Interviews: "How often does this happen?" | Define from the segment's natural cadence |
| **Motivation** (pain severity) | Consequence and current behavior | Compare with a documented baseline |
| **Expected Outcome** (value) | Behavioral commitment test | Predefine a channel-specific effect/precision target |
...
**Iteration rule:** Start with a provisional job or with observed problem evidence, then refine both through research. Preserve traceability; do not force one universal order.
...
**Freemium rules:**
- If a free tier is part of the strategy, define which value hypothesis it tests and how it avoids misleading or trapping users.
...
LTV:CAC Ratio:
  ...
  Interpret the ratio using gross margin, payback period, retention,
  cash constraints, channel maturity, and uncertainty. No range is
  universally "healthy."
...
**PMF survey heuristic:** The “very disappointed” question can be one diagnostic signal. Do not use a fixed percentage as universal proof of product-market fit.
...
- **Generic Subdomains:** Evaluate buy, build, partner, or adapt using regulation, data control, reliability, integration, total cost, vendor health, and exit strategy.
